file = "outputs/cal_time-.csv"
dat = read.csv(file)
 
 rcps = c("2.6" = "rcp2_6", "8.5" = "rcp8_5")
 degs = c("None" = 0, "1_5" = 1.5, "2_deg" = 2, "4_deg" = 4)
 ...
 cal_plot_data <- function(rcp, deg) {
	...
	index0 = which(grepl(rcp, colnames(dat)) & grepl("None", colnames(dat)))
	out = dat[index] 
	
	#if (diff) out = out - dat[index0]
	#else {
		out = out[nrow(out):1,]
...
png("time_in_ffdi_range.png", height = 5, width = 7.2, units = 'in', res = 300)
par(mar = c(2, 3, 0.1, 2.2))
...
lapply(1:length(degs), function(j) 
						lapply(1:length(rcps), plot_rcpDeg, j))
...
text(x = xrange[1], y = 1, '2004-\n2023', 
	 srt = 90, xpd = TRUE, adj = c(0.5, -0.5))
text(x = xrange[1], 
	 y =  0.5+ (length(rcps) + 2.5) * (1:(length(degs)-1)), 
	 degs[-1], 
